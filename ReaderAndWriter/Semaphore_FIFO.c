...
sem_t lockSem; // 保护共享资源锁
sem_t mutexSem; // 互斥锁，保护读者计数器
int readCount = 0; // 读者计数器，记录当前有多少读者正在读取数据

void* WriteThread(void* arg)
{
    sem_wait(&queueSem); // 请求排队锁，按 FIFO 顺序排队
    sem_wait(&lockSem); // 请求资源锁，保护共享数据
    sem_post(&queueSem); // 允许下一个线程进入排队
...
void* ReadThread(void* arg)
{
    ...
    sem_post(&mutexSem); // 允许其他读者增加计数器
    sem_post(&queueSem); // 允许下一个线程进入队列
    printf("当前共 %d 个读者，线程读取到:%s\n", readCount, data.msg);
    sem_wait(&mutexSem); // 再次保护读者计数器
    readCount--; 
